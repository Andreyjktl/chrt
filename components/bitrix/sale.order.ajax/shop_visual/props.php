...
<script type="text/javascript">
	...
	<?
	//BL установим телефон из профиля{
	$rsUser = CUser::GetByID($USER->GetID());
	$arUser = $rsUser->Fetch();
	$user_phone = $arUser["PERSONAL_PHONE"];
	//}
	?>
	//BL Скроем поле местоположения {
	$(document).ready(function() {
		$("input[name='ORDER_PROP_6'], input[name='ORDER_PROP_18']").parents("tr").hide();
		if ($("input[name='ORDER_PROP_3']").val() == "" || $("input[name='ORDER_PROP_14']").val() == "")
			$("input[name='ORDER_PROP_3'], input[name='ORDER_PROP_14']").val("<?=$user_phone?>");
	});
