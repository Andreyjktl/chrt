					...
					<!--noindex-->
						<?if( $arItem["CAN_BUY"] || count($arItem["OFFERS"])){?>
							<div class="button_block">
								...
								<a rel="nofollow" element_id="#<?=$arItem["ID"]?>" 
							href="<?=$arItem["DETAIL_PAGE_URL"]?>?action=ADD2BASKET&id=<?=$arItem["ID"];?>" 
							onclick="return addToCartList(this, 'detail', '<?=GetMessage("CATALOG_IN_CART")?>', 'cart', '<?=$arParams["BASKET_URL"]?>', '<?=$arItem["ID"]?>');" class="button add_item" alt="<?=$arItem["NAME"]?>"><span><?=GetMessage('CATALOG_ADD_TO_BASKET')?></span></a>
								<?else:?>
